...
int init(ProcessParam *process_param)
{
  ...
  if (process_param->is_demon()) {
    rc = daemonize_service(process_param->get_std_out().c_str(), process_param->get_std_err().c_str());//输出和错误日志重定向
    if (rc != 0) {
  ...
  writePidFile(process_param->get_process_name().c_str());//进程信息的写入，写到进程名相关文件下

  // Initialize global variables before enter multi-thread mode
  // to avoid race condition

  // Read Configuration files
  rc = get_properties()->load(process_param->get_conf());//加载配置文件
  if (rc) {
  ...
  // Init tracer
  rc = init_log(process_param, *get_properties());//根据程序名初始化日志，日志地址是绝对路径
  if (rc) {
