...
Server *init_server()
{
  std::map<std::string, std::string> net_section = get_properties()->get(NET);//这是一个Ini对象，get返回的是一个kv对

  ProcessParam *process_param = the_process_param();

  long listen_addr = INADDR_ANY; //ip地址，这里是广播形式
  long max_connection_num = MAX_CONNECTION_NUM_DEFAULT;//最大连接数
  int port = PORT_DEFAULT;//默认通信端口
...
int main(int argc, char **argv)
{
  ...
  setSignalHandler(quit_signal_handle);//信号的处理函数注册

  parse_parameter(argc, argv);//命令行参数解析

  rc = init(the_process_param());//配置文件加载，初始化日志，初始seda（g工厂模式，再看）
